...
(function( $ ) {
  ...
  var methods = {
    init : function( options ) {
      ...
      settings = $.extend( {
        ...
        datetimeFormat  : {
          ...
          months        : "F", 
          days          : "d/m/Y",
          meta          : "Y/m/d H:i", // start datetime in meta
      ...
      return this.each(function(){
        ...
        var $this = $(this),
            ...
            timeline = $('<div />', {
              ...
              "datetime-format-days"  : settings.datetimeFormat.days || 'd/m/Y',
    ...
    render : function( options ) {
      ...
      return this.each(function(){
        ...
        // update options
        if ( 'rowTitles' in options ) {
	      data.timeline.attr( 'row-titles', options.rowTitles );
        }
        if ( 'type' in options ) {
    ...
    updatePointer : function () {
	    reRender = false;
		var tlPointer = $('.timeline-needle-pointer');
		var $this = $(this);
		var data = $this.data('timeline');
    ...
    showIcon : function (evId, icon, iconType, toolTip = '') {
	    reRender = false;
	    if (iconType == 'alert') {
		    $('#evt-' + evId).css('border', '2px solid #ba4a00');
		    $('#evt-icon-' + evId).css('border', '2px solid #ba4a00');
		    $('#evt-icon-' + evId).css('background', '#e74c3c');	    
		    $('#evt-icon-' + evId).html('<i class="tl-event-icon fa fa-fw faa-flash faa-fast animated fa-' + icon + '"></i>');
		    $('#evt-icon-' + evId).css('visibility', 'visible');
		    if (toolTip != '') {
		    	var originalTitle = $('#evt-' + evId).attr('data-defaulttitle');
				var newTitle = originalTitle + ' - ' + toolTip;
				$('#evt-' + evId).attr('data-original-title', newTitle);
			}
		}		
	    if (iconType == 'warning') {
		    $('#evt-' + evId).css('border', '2px solid #fbc02d');
		    $('#evt-icon-' + evId).css('border', '2px solid #fbc02d');
		    $('#evt-icon-' + evId).css('background', '#ffca28');	    
		    $('#evt-icon-' + evId).html('<i class="tl-event-icon fa fa-fw faa-flash faa-slow animated fa-' + icon + '"></i>');
		    $('#evt-icon-' + evId).css('visibility', 'visible');
		    if (toolTip != '') {
		    	var originalTitle = $('#evt-' + evId).attr('data-defaulttitle');
				var newTitle = originalTitle + ' - ' + toolTip;
	    ...
	    if (iconType == 'info') {
		    $('#evt-' + evId).css('border', '2px solid  #1e8449 ');
		    $('#evt-icon-' + evId).css('border', '2px solid  #1e8449 ');
		    $('#evt-icon-' + evId).css('background', '#229954');	    
		    $('#evt-icon-' + evId).html('<i class="tl-event-icon fa fa-fw fa-' + icon + '"></i>');
		    $('#evt-icon-' + evId).css('visibility', 'visible');
		    if (toolTip != '') {
		    	var originalTitle = $('#evt-' + evId).attr('data-defaulttitle');
				var newTitle = originalTitle + ' - ' + toolTip;
    ...
    hideIcon : function (evId) {
	    reRender = false;
	    $('#evt-' + evId).css('border', '2px dotted grey');
	    $('#evt-icon-' + evId).html('');
	    $('#evt-icon-' + evId).css('visibility', 'hidden');
	    var originalTitle = $('#evt-' + evId).attr('data-defaulttitle');
	    $('#evt-' + evId).attr('data-original-title', originalTitle);
	    return true;	    
    },
    addEvent : function( events, callback ) {
      return this.each(function(){
        var $this       = $(this),
            ...
            eventNodes = returnEventString(data),
            incrementId = 1,
            _ids        = [ incrementId ];
        // add events
        if ( events.length > 0 ) {
  ...
  function renderTimeline( obj ) {
    ...
    // Create header
    if ( data.timeline.attr('show-headline') == 1 ) {
      ...
      tlHeader.append('<h3 class="timeline-headline">' + tlTitle + '</h3>');
	    ...
	    tlHeader.append('<h3 class="timeline-headline">&nbsp;</h3>');
    }
    ...
    var tTitles = '<div id="timelineTitles" class="timeline-row-titles"><ul class="row-titles">';
    
    var tmpTitles = data.timeline.attr( 'row-titles' ).split(',');
    var arrTitles = JSON.stringify(tmpTitles);
    arrTitles = JSON.parse(arrTitles);
    
    arrTitles.forEach(function(t){
		tTitles = tTitles + '<li class="row-title">' + t + '</li>'; 
    });
    tTitles = tTitles + '</ul></div>';
  ...
  function placeEvents( obj ) {
    ...
    //console.log(returnEventString(data));
        ...
        if ( coordinate.w > 0 ) {
          if ( tlType === 'point' ) {
            ...
            tlNodeElm = $('<div />', {
              ...
              //text: evt.label,
              html: evt.label + '<span class="tl-event-icon-container" id="evt-icon-' + evt.eventId + '"></span>'
            });
            if ( evt.color ) {
    ...
    if ( tlType === 'point' || tlType === 'mixed' ) {
      ...
      $this.find('.timeline-event-pointer').hover(function(e){
		...
		$this.find('.timeline-text-truncate').hover(function(e) {

			var eventId = Number((e.currentTarget.id).replace('evt-', ''));
			...
			if ( e.type === 'mouseenter' ) {
				...
				if (eventData.onhover) {
					Function.call(null, 'return ' + eventData.onhover)();
				}
			...
			if ( e.type === 'mouseleave' ) {
				...
				if (eventData.offhover) {
					Function.call(null, 'return ' + eventData.offhover)();
				}
	...
	// Automatically trigger bootstrap popovers
  ...
  function returnEventString (data) {
	  
	  var tlEventString = data.timeline.text();
	  	  
	  if (tlEventString == '') {
		  return new Array();
	  }
	  else {
		  return ( new Function( 'return ' + data.timeline.text() ) )();
	  }
  ...
  function showEvent( eventData, metaFormat ) {
    ...
    temp =  '<span class="timeline-event-start-date">' + formatDate( 'H:i', eventData.start ) + '</span>';
    if ( eventData.end ) {
      temp += '<span class="timeline-event-date-separator"></span>';
      temp += '<span class="timeline-event-end-date">' + formatDate( 'H:i', eventData.end ) + '</span>';
    }
